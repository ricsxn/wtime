...
#
    ...
    def calc(self):
        ...
        if self.overtime == True:
            remaining_or_overtime_key = 'overtime'
            remaining_or_overtime_value = self.getOverTime()
        else:
            remaining_or_overtime_key = 'remaining_time'
            remaining_or_overtime_value = self.getRemainingTime()
        # json data representation
        self.json_data = {
            'start_time': self.getStartTime(),
            'working_time': self.getWorkingTime(),
            'pause_time': self.getPauseTime(),
            'current_time': self.getCurrentTime(),
            'current_elapse': self.getCurrentElapse(),
            'due_time': self.getDueTime(),
            'to_end': self.to_end,
            remaining_or_overtime_key: remaining_or_overtime_value,
            'ticket_time': self.getTicketTime(),
            'ticket_at': self.getTicketAt(),
            'to_ticket': self.to_ticket,
            'ticket_remain': self.getTicketRemain()}
    ...
    def __repr__(self):
        self.calc()
        return json.dumps(self.json_data,4)
            ...
            if opt_key is not None:
                ...
                if arg[0] == '-':
                    ...
                    if arg == '-h' or arg == '--help':
                        wtime.show_usage(wtimecmd)
                    elif arg == '-c' or arg == '--current-time':
